...
SAVE_PATH='./scripts/runs/f30k_GPO_butd_woft'
CUDA_VISIBLE_DEVICES=0 python3 train.py \
  --data_path=${DATA_PATH} --data_name=${DATASET_NAME}  --vocab_path=${VOCAB_PATH}\
  --logger_name=${SAVE_PATH}/log --model_name=${SAVE_PATH} \
  --num_epochs=25 --lr_update=15 --learning_rate=5e-4 --precomp_enc_type=backbone  --workers=16 --backbone_source=detector \
  --vse_mean_warmup_epochs=1 --backbone_warmup_epochs=100 --embedding_warmup_epochs=100  --optim=adam --backbone_lr_factor=0.01 \
  --input_scale_factor=2.0  --backbone_path=${BUTD_WEIGHT_PATH}/original_updown_backbone.pth --log_step=200 --batch_size=64 \
  --coding_type=VHACoding --alpha=0.1 --pooling_type=LSEPooling --belta=0.1 \
  --drop --wemb_type=glove --criterion=ContrastiveLoss --margin=0.05 \
...
CUDA_VISIBLE_DEVICES=0 python3 train.py \
  ...
  --num_epochs=25 --lr_update=15 --learning_rate=5e-4 --precomp_enc_type=backbone  --workers=16 --backbone_source=detector \
  --vse_mean_warmup_epochs=1 --backbone_warmup_epochs=100 --embedding_warmup_epochs=100  --optim=adam --backbone_lr_factor=0.01 \
  --input_scale_factor=2.0  --backbone_path=${BUTD_WEIGHT_PATH}/original_updown_backbone.pth --log_step=200 --batch_size=128 \
  --coding_type=VHACoding --alpha=0.1 --pooling_type=MeanPooling --belta=0.1 \
  --drop --wemb_type=glove --criterion=ContrastiveLoss --margin=0.2 --no_imgnorm --no_txtnorm \
...
SAVE_PATH='./scripts/runs/coco_GPO_butd_woft'
CUDA_VISIBLE_DEVICES=0 python3 train.py \
  --data_path ${DATA_PATH} --data_name ${DATASET_NAME}  --vocab_path ${VOCAB_PATH}\
  --logger_name=${SAVE_PATH}/log --model_name=${SAVE_PATH} \
  --num_epochs=25 --lr_update=15 --learning_rate=5e-4 --precomp_enc_type=backbone  --workers=16 --backbone_source=detector \
