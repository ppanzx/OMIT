...
from lib.modules.utils import set_seeds
...
def main():
    ...
    for epoch in range(start_epoch, opt.num_epochs):
        ...
        if opt.precomp_enc_type in ["backbone","clip"]:
...
def train(opt, train_loader, model, epoch, val_loader):
    ...
    if epoch<1:
        ...
        logger.info('text encoder trainable parameters: {}'.format(count_params(model.txt_enc)))
    ...
    for i, train_data in enumerate(train_loader):
        ...
        if opt.precomp_enc_type in ["backbone","clip"] and epoch == opt.embedding_warmup_epochs:
            warmup_alpha = float(i) / num_loader_iter
            model.train_emb(images, captions, lengths, image_lengths=img_lengths, warmup_alpha=warmup_alpha)
        else:
