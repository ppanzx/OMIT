...
def encode_data(model, data_loader, log_step=10, logging=logger.info):
    ...
    for i, data_i in enumerate(data_loader):
        ...
        max_txt_len = max(lengths) # max length of texts in mini-batch
        # compute the embeddings
        img_emb, cap_emb, image_lengths, lengths = model.forward_emb(images, captions, lengths, image_lengths=image_lengths)
        ...
        if cap_emb.dim() == 3:
            cap_embs[ids,:max_txt_len,:] = cap_emb.data.cpu().numpy().copy()
        else:cap_embs[ids] = cap_emb.data.cpu().numpy().copy()
...
def evalrank(model_path, data_path=None, split='dev', fold5=False, save_path=None, cxc=False):
    ...
    opt.backbone_path="/media/panzx/新加卷/PanZhengxin/datasets/CrossModalRetrieval/weights/original_updown_backbone.pth"
    opt.vocab_path = '/media/panzx/新加卷/PanZhengxin/datasets/CrossModalRetrieval/vocab'
